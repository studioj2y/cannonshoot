...
| 6 | Twin Towers / 双塔 | 39（38 可动 + 1 静态平台） | 5 | 1700 | 砸倒 14 块 | 草地 |
| 7 | Stepped Fortress / 阶梯要塞 | 38（36 可动 + 2 静态平台） | 5 | 2000 | 砸掉 2 颗紫色目标 | 草地 |
| 8 | Twin-Tier Colonnade / 双层柱廊 | 39（38 可动 + 1 静态平台） | 5 | 2400 | 砸倒 19 块 | 沙地 |
| 9 | Twin Keeps / 双砦 | 55（54 可动 + 1 静态平台） | 6 | 2500 | 砸倒 20 块 | 草地 |
| 10 | The Cathedral / 大教堂 | 66（65 可动 + 1 静态平台） | 6 | 3800 | 砸倒 28 块 | 沙地 |
...
|---|---|---|---|---|---|---|---|
| 1 | 26 | 8 | 31% | 5 | 22 | 275% | 13.8× |
| 2 | 19 | 1（目标物） | — | 5 | 19 | 1900% | 95.0× |
| 3 | 22 | 14 | 64% | 4 | 22 | 157% | 6.3× |
| 4 | 16 | 4（红砖） | 25% | 5 | 10 | 250% | 12.5× |
| 5 | 51 | 30 | 59% | 6 | 41 | **137%** | 8.2× |
| 6 | 38 | 14 | 37% | 5 | 19 | **136%** | 6.8× |
| 7 | 36 | 2（目标物） | — | 5 | 14 | 700% | 35.0× |
| 8 | 38 | 19 | 50% | 5 | 38 | 200% | 10.0× |
| 9 | 54 | 20 | 37% | 6 | 28 | **140%** | 8.4× |
| 10 | 65 | 28 | 43% | 6 | 40 | 143% | 8.6× |

> **这张表改过一次。** 初稿 L6 目标 18、L9 目标 26 是按「目标占可击倒砖数的比例」定的（47% / 48%，落在老五关 25%~64% 的带里）。实测后发现这个比例是个**不充分的判据**：L6 是「两座独立的塔 + 一块搭在两塔上的长板」，一发最多打掉其中一座塔加板上的东西（实测 19 块）；它不可能像 L5 那样一发烧掉大半场。所以「一发/目标」只有 106%，比老五关里最紧的 L5（137%）还紧 —— 玩家必须打出一发近乎完美的球才能一发过关。降到 14 后是 136%，与 L5 持平。
>
> **教训：配关卡不能只看「目标占全场几成」，要看「最强的一发能覆盖几成」。** 结构越分散（多个独立承重单元），单发覆盖率越低，目标数就得跟着降。
...
│   ├── levels.ts        (385)   10 关数据（中英对照）+ wall/tower/arch 构件工厂 + 摆砖硬约束说明
...
├── check-levels.mjs     (211)   改完 levels.ts 必跑：摆放静检 + 开局空跑 6 秒查自倒数
├── check-levels-live.mjs(265)   真机回归：需先起静态服务 + headless Chrome；查选关面板与 L6~L10
└── playtest.mjs         (373)   难度校准：瞄准解算候选弹道，扫出「一发最好」等指标
```

**改完关卡怎么验**（`levels.ts` 的摆砖约束踩过两次坑，一律先跑工具再谈手感）：

```bash
node tools/check-levels.mjs        # 应该 10/10 全绿：摆放全贴合 + 自倒 0 块
node tools/playtest.mjs            # 可选：看难度是不是还在同一带里（10 关约 25 分钟）
node tools/check-levels-live.mjs   # 可选：改过选关/HUD 布局时跑，需先起服务与浏览器
```

三个脚本都把 `Game.ts` 的物理常量（重力 / 求解器 / 四组接触材质 / 阻尼 / 击倒阈值 / 炮弹寿命与上限）
逐条抄了一份 —— **改 `Game.ts` 里的物理参数时，记得同步前两个文件顶部的常量区**。
