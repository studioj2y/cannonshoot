 ...
 *
 * 覆盖：教程页 → 用 Esc 打开暂停面板 → 逐关点进 L6~L10 → 每关等 3.5 秒确认
 * 「第 N 关 / 10」与开局进度 `0 / N`（开局自倒会让进度非零）→ 选关面板确有 10 个按钮
 * → 再切到 412×915 手机档重复一遍 → 检查窄屏横向溢出与控制台报错。
 *
...
await shot('C10-02-levelselect-desktop');
...
await escapeKey();
await sleep(600);
await shot('C10-03-levelselect-mobile');
